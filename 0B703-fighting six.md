...
尽快申请身份认证。如果入学时选择迁户口，身份认证在办理迁户口后、新身份证办理下来前极有可能不通过。 
 
2.GitHub 资源下载速度慢，下载经常暂停： 

换个时间进行下载，与网络、网站有关。建议早上下载。 
 
3.auth 认证中，树莓派默认浏览器登录百度账号出现"not_login"错误，同时详细描述中显示"未知错误"： 

浏览器 cookie 配置问题，在浏览器-首选项中将 cookie 设置为永不阻止状态即可成功登录。 
 
4.唤醒词替换错误： 

如果你在网上查找到 DuerOS 唤醒词替换的教程，按照做时却出现了失败，建议不要进行唤醒词替换的工作。因为我们所使用的镜像并不是
DuerOS 的官方镜像，唤醒词替换时出现的问题，暂时没有找到合适的解决办法。 
